...
SpringCloud使用的技术：Spring Cloud Gateway、Nacos、OpenFeign、RabbitMQ 等

MyBatis-Plus **3.5.5**、API 文档 **springdoc-openapi**、JSON **fastjson2**

详细版本见 [`../docs/TECH_STACK.md`](../docs/TECH_STACK.md)
...
> 推荐从仓库根目录 [`../README.md`](../README.md) 使用 `docker compose` 与 `.env` 启动基础设施。

运行环境：Redis、Nacos、RabbitMQ、MySQL、MongoDB（可用根目录 `docker compose up -d` 一键启动）

数据初始化：`sql/` 目录下有 MySQL 脚本（Docker 首次启动会自动导入）；MongoDB 数据需在运行 `hospital-manage` 后访问 http://localhost:9998/ 导入。
...
- `yygh-site` 用户预约端 — http://localhost:3000/
- `hospital_admin` 医院管理端 — http://localhost:9528/

**API 网关** — http://localhost:8080/（端口由环境变量 `GATEWAY_PORT` 控制，默认 8080）

**后端微服务**（`service/` 目录）：

- `service_cmn` 字典、`service_hosp` 医院、`service_msm` 短信、`service_order` 订单
- `service_oss` 文件、`service_statistics` 统计、`service_task` 定时、`service_user` 用户

**其他**：

- `service_gateway` 网关
- `hospital-manage` 医院对接模拟（:9998）
- `service_client` Feign 客户端