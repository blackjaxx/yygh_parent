...
public class WeixinApiController {
    ...
    public Result genQrConnect(HttpSession session) throws UnsupportedEncodingException {
        ...
        map.put("state", System.currentTimeMillis() + "");// 注意：生产环境应将state存入Redis并在回调时验证，以防范CSRF攻击
    ...
    public String callback(String code, String state) {
        ...
        log.info("微信授权获取access_token完成");

        JSONObject resultJson = JSONObject.parseObject(result);
        if (resultJson.getString("errcode") != null) {
            log.error("获取access_token失败：errcode={}, errmsg={}",
                    resultJson.getString("errcode"), resultJson.getString("errmsg"));
            throw new YyghException(ResultCodeEnum.FETCH_ACCESSTOKEN_FAILD);
        }

        String accessToken = resultJson.getString("access_token");
        String openId = resultJson.getString("openid");
        // 敏感凭证仅输出前几位用于调试，生产环境应关闭此类日志
        log.info("获取到access_token（已脱敏），openId={}", openId);
        ...
        map.put("token", token);
        // 跳转到前端页面，对所有URL参数进行编码避免特殊字符导致参数截断
        String nameStr = map.get("name") != null ? URLEncoder.encode(map.get("name").toString(), "UTF-8") : "";
        return "redirect:" + ConstantPropertiesUtil.YYGH_BASE_URL
                + "/weixin/callback?token=" + URLEncoder.encode(map.get("token").toString(), "UTF-8")
                + "&openid=" + URLEncoder.encode(map.get("openid").toString(), "UTF-8")
                + "&name=" + nameStr;
    }
}
