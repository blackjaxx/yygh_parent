...
public class PaymentServiceImpl extends
    ...
    /**
     * 支付成功处理
     * 将本地数据库操作放在事务内，远程HTTP调用放在事务外，避免长时间占用数据库连接
     */
    @Override
    public void paySuccess(String outTradeNo, Integer paymentType, Map<String, String> paramMap) {
        ...
        // 阶段一：本地数据库事务操作
        this.updatePaymentAndOrder(outTradeNo, paymentInfo, paramMap);

        // 阶段二：远程调用通知医院更新支付状态（事务外执行）
        ...
        if (result.getInteger("code") != 200) {
            log.error("通知医院支付状态失败, outTradeNo={}, result={}", outTradeNo, result);
    ...
    public void updatePaymentAndOrder(String outTradeNo, PaymentInfo paymentInfo, Map<String, String> paramMap) {
        // 修改支付状态
